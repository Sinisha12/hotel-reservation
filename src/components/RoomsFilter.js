...
const getAllUniqueValue = (items, value) => {
...
function RoomsFilter({rooms}) {
    ...
    let types = getAllUniqueValue(rooms, 'type')    
    types = ["all", ...types]
    types = types.map((type,index) => <option value={type} key={index} >{type}</option>)

    let people = getAllUniqueValue(rooms, 'capacity')   
    people = people.map((person,index) => <option value={person} key={index} >{person}</option>)
    return (
